...
namespace QL.Core.Net
{
    ...
    public class Parameters
    {
        ...
        {
            return BuildQueryString(true, Encoding.UTF8);
        }
        /// <summary>
        /// 构造查询参数字符串,字符格式如: name1=value1&amp;name2=value2
        /// </summary>
        ...
        /// <returns></returns>
        public string BuildQueryString(Encoding encoding)
        {
            return BuildQueryString(true, encoding);
        }
        /// <summary>
        /// 构造查询参数字符串,字符格式如: name1=value1&amp;name2=value2
        /// </summary>
        ...
        /// <returns></returns>
        public string BuildQueryString(bool encodeValue, Encoding encoding)
        {
            StringBuilder buffer = new StringBuilder();
            foreach (var p in this.Items)
            {
                if (buffer.Length != 0) buffer.Append("&");
                buffer.AppendFormat("{0}={1}", encodeValue ? HttpUtility.UrlEncode(p.Key, encoding) : p.Key, encodeValue ? HttpUtility.UrlEncode(p.Value, encoding) : p.Value);
            }
